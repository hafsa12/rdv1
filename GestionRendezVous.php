...
class GestionRendezVous extends AtexoPage {
	...
	private function envoiMail($rdv)
	{
		...
		//$corpsMessage .= "<li>".prado::localize('MOTIF_ANNULATION')." : ".$rdv->getMotifAnnulation()."</li>";
	...
	public function exporterExcel() {
		...
		$etiq->writeString(0, 5,$dateRdv, $hdr_format);
		$etiq->writeString(0, 6,$heureDebut, $hdr_format);
		$etiq->writeString(0, 7,$heureFin, $hdr_format);
		$etiq->writeString(0, 8,$etab, $hdr_format);
		$etiq->writeString(0, 9,$type, $hdr_format);
		$etiq->writeString(0, 10,$prestation, $hdr_format);
		$etiq->writeString(0, 11,$objetRdv, $hdr_format);
		$etiq->writeString(0, 12,$ressource, $hdr_format);
		$etiq->writeString(0, 13,$etat, $hdr_format);
		$etiq->writeString(0, 14,$annuleLe, $hdr_format);
		$etiq->writeString(0, 15,$annulePar, $hdr_format);

		$etiq->writeString(0, 16,$nomClient, $hdr_format);
		...
		$etiq->writeString(0, 19,$identifiant, $hdr_format);
		$etiq->writeString(0, 20,$administration, $hdr_format);
		...
		foreach($dataRdv as $data) {
			...
			$etiq->writeString($c,5,$data["DATE_DEB_RDV"],$corps_format);
			$etiq->writeString($c,6,$data["TIME_DEB_RDV"],$corps_format);
			...
			$etiq->writeString($c,9,$data["TYPE_PRESTATION"],$corps_format);
			$etiq->writeString($c,10,$data["PRESTATION"],$corps_format);
			$etiq->writeString($c,11,$objet,$corps_format);
			$etiq->writeString($c,12,$data["NOM_RESSOURCE"]." ".$data["PRENOM_RESSOURCE"],$corps_format);
			...
			$etiq->writeString($c,16,$data["NOM"]." ".$data["PRENOM"],$corps_format);
			...
			$etiq->writeString($c,19,$data["IDENTIFIANT"], $corps_format);
			$etiq->writeString($c,20,$data["RAISON_SOCIAL"], $corps_format);
...
if($etab==109 ) $path = "ressources/word/recap_rdv_52_ar"  . ".docx";
 else       $path = "ressources/word/recap_rdv_". $idOrg . "_" . $lang . ".docx";
        $vendorpath = 'protected/library/vendors/vendor';
        require_once $vendorpath.'/autoload.php';
        if(!file_exists($path)){
				if($etab==109 ) $path = "ressources/word/recap_rdv_52" . $lang . ".docx";
		...
		try {
			if(isset($rdv->getTCitoyen()->getText2())){
				$templateProcessor->setValue('denom', $rdv->getTCitoyen()->getText2());
				$templateProcessor->setValue('nom_citoyen', '');
			}
			else {
				$templateProcessor->setValue('denom', '');
				$templateProcessor->setValue('nom_citoyen', $rdv->getTCitoyen()->getNom());
			}
			if(isset($rdv->getTCitoyen()->getText1())) $templateProcessor->setValue('rc', $rdv->getTCitoyen()->getText1());
